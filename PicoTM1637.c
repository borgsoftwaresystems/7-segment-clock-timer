...
void TM1637_display_dash_and_number(int leftNum, int rightNum, int posToDash, bool leadingZeros) {
  ...
  if (posToDash == 1) {
    leftHex = fetch_char_encoding('-');
    leftHex = leftHex + (leftHex << 8); //Twice
    leftHex |= 0x8000; //:
    rightHex = two_digit_to_segment(rightNum, leadingZeros, false); 
  }
  else {
    leftHex = two_digit_to_segment(leftNum, leadingZeros, false);
    leftHex |= 0x8000; //:
    rightHex = fetch_char_encoding('-');
    rightHex = rightHex + (rightHex << 8); //Twice
  }
